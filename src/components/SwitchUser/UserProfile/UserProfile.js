...
const UserProfile = ({
  ...
  if (profileImage === "bard") {
    ...
    profile = (
      <img className="UserProfile__profile" src={sorceress} alt="sorceress" />
    );
  } else if (profileImage === "ranger") {
    profile = (
      <img className="UserProfile__profile" src={ranger} alt="ranger" />
    );
  } else if (profileImage === "monk") {
    profile = <img className="UserProfile__profile" src={monk} alt="monk" />;
  } else if (profileImage === "wizard") {
    profile = (
      <img className="UserProfile__profile" src={wizard} alt="wizard" />
    );
  }
  ...
  if (favoriteActivity === "running") {
    activity = (
      <img className="UserProfile__icon" src={running} alt="running" />
    );
  } else if (favoriteActivity === "swimming") {
    activity = (
      <img className="UserProfile__icon" src={swimming} alt="swimming" />
    );
  } else if (favoriteActivity === "walking") {
    activity = (
      <img className="UserProfile__icon" src={walking} alt="walking" />
    );
  } else if (favoriteActivity === "weights") {
    activity = (
      <img className="UserProfile__icon" src={weights} alt="weights" />
    );
    ...
    <>
      {name !== "empty" ? (
        <Link
          ...
          className={
            optionSelected === index
              ? "UserProfile UserProfile__selected"
              : "UserProfile"
          }
        ...
        <div
          className={
            optionSelected === index
              ? "UserProfile UserProfile__disabled"
              : "UserProfile"
          }
          onMouseOver={() => selectProfile(index)}
        >
