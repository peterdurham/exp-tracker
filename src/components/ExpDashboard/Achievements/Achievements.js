...
const Achievements = ({activeUser, optionSelected, highlightOption}) => {
    const tasks = ['coding', 'cookfood', 'read', 'socialize', 'weights', 'meetup', 'journalling', 'meditation', 'pairprogramming',
        'completeproject', 'brushteeth', 'paybills', 'plannewproject','cleanroom', 'deployproject', 'running'];
    let tasksCompleted = Object.assign([], activeUser.tasksCompleted)
    let combined = tasks.map(task=>{
        let amount = tasksCompleted.filter((instance) => instance.name === task).length;
    ...
    return(
        <div className="Achievements">
            ...
            <div className="Achievements__tasks">
                {completedFive.map((task, index) => <AchievementTask 
                    name={task.name}
                    key={index}
                    times={task.times}
                />)}
            ...
            <div className="Achievements__tasks">
                {completedTen.map((task, index) => <AchievementTask 
                    name={task.name}
                    key={index}
                    times={task.times}
                />)}
            ...
            <div className="Achievements__tasks">
                {completedTwentyfive.map((task, index) => <AchievementTask 
                    name={task.name}
                    key={index}
                    times={task.times}
                />)}
